...
void moveFrontShort(int distance) {
  ...
  int i;
  for (i = 100; (M1Ticks < distance && i < MSpeed); i += 50) {
    pid = computePID10CM();
...
double computePID10CM() {
  ...
  kp = 4.7;//2.4//2.1;//2.2 (FOR 300);//1.35;// (SLIGHTLY RIGHT)//1.4 (TOO LOW STILL LEFT)//1.5 (WORKING FOR 150m) //2228;
  ki = 0.1;//0.1;//0//0//.11;//0.8;
  kd = 0.25;//0.2;//0.2//0//0.1;
...
void rotate(int degree, double distance, boolean isRight) {
  ...
  if (isRight) {
    ...
    while (M1Ticks < distance + L_offset) {
      pid = computePID_right(degree);
      int M1setSpeed = (MSpeedR);
      int M2setSpeed = -(MSpeedR + pid);
      md.setSpeeds (M1setSpeed, M2setSpeed);
...
const int rFL []PROGMEM = {99, 213, 344};
const int rFM []PROGMEM = {76, 181, 313};
const int rFR []PROGMEM = {98, 207, 347};
const int rLF []PROGMEM = {101, 214, 328};
const int rLB []PROGMEM = {110, 232, 354};
const int rR  []PROGMEM = {188, 234, 335, 449, 573};
